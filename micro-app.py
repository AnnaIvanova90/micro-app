...
blurred_img = cv2.GaussianBlur(gray, ksize=(11, 11), sigmaX =0, sigmaY=0)
thresh = cv2.adaptiveThreshold(blurred_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 21, 50)
ret,thresh1 = cv2.threshold(blurred_img,100,255,cv2.THRESH_OTSU)
...
for n in nn:
    r = ((n*4/3.14)**0.5)*coef
    if r > 14 and r < 10000: Req.append(r)
D = round(max(Req),2)
